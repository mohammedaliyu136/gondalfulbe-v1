@php
    ...
    $canAccessGondal = Gate::check('manage vender') || GondalPermissionRegistry::canAccessAny(Auth::user());
    $accountDashboardUrl = \Illuminate\Support\Facades\Route::has('dashboard')
        ? route('dashboard')
...
<div class="navbar-wrapper">
    <div class="m-header main-logo" style="height: 96px; padding: 12px 20px;">
        <a href="#" class="b-brand">
            ...
            @if ($setting['cust_darklayout'] && $setting['cust_darklayout'] == 'on')
                <img src="{{ $logo . '/' . (isset($company_logos) && !empty($company_logos) ? $company_logos : 'logo-dark.png') . '?' . time() }}"
                    alt="{{ config('app.name', 'Gondal Fulbe') }}" class="logo logo-lg" style="height: 72px; width: auto; max-height: none;">
            @else
    ...
    <div class="navbar-content">
        @if (\Auth::user()->type != 'client')
            <ul class="dash-navbar">
                ...
                @if (Gate::check('show hrm dashboard') ||
                    ...
                    <li
                        class="dash-item dash-hasmenu
                                ...
                                Request::segment(1) == 'gondal-dashboard' ||
                                    ...
                                    <ul class="dash-submenu">
                                        @can('show account dashboard')
                                            <li
                                                ...
                                                <a class="dash-link"
                                                    href="{{ $accountDashboardUrl }}">{{ __(' Overview') }}</a>
                                            </li>
                                        @endcan
                ...
                <!--------------------- Start Agricultural ----------------------------------->
                @if ($canAccessGondal)
                    <li class="dash-item {{ request()->routeIs('gondal.dashboard*') ? 'active' : '' }}">
                        <a href="{{ route('gondal.dashboard') }}" class="dash-link">
                            <span class="dash-micon"><i class="ti ti-home"></i></span>
                            <span class="dash-mtext">{{ __('Gondal Dashboard') }}</span>
                        </a>
                    </li>
                    @php
                        $agriculturalModules = [
                            ...
                            ['route' => 'gondal.reports', 'active' => request()->routeIs('gondal.reports*'), 'icon' => 'ti-report-money', 'label' => 'Reports', 'module' => 'reports'],
                        ];
                    @endphp

                    @foreach ($agriculturalModules as $module)
                        @if ((isset($module['permission']) && Gate::check($module['permission'])) || (isset($module['module']) && GondalPermissionRegistry::canAccessModule(Auth::user(), $module['module'])))
                            <li class="dash-item {{ $module['active'] ? 'active' : '' }}">
                                <a href="{{ route($module['route']) }}" class="dash-link">
                                    <span class="dash-micon"><i class="ti {{ $module['icon'] }}"></i></span>
                                    <span class="dash-mtext">{{ __($module['label']) }}</span>
                                </a>
                            </li>
                        @endif
                    @endforeach
